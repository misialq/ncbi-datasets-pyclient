# V2reportsSequenceDataReportMatch


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**query** | **List[str]** |  | [optional] 
**sequence** | [**V2reportsSequenceDataReport**](V2reportsSequenceDataReport.md) |  | [optional] 
**errors** | [**List[V2reportsError]**](V2reportsError.md) |  | [optional] 

## Example

```python
from ncbi.datasets.openapi.models.v2reports_sequence_data_report_match import V2reportsSequenceDataReportMatch

# TODO update the JSON string below
json = "{}"
# create an instance of V2reportsSequenceDataReportMatch from a JSON string
v2reports_sequence_data_report_match_instance = V2reportsSequenceDataReportMatch.from_json(json)
# print the JSON string representation of the object
print(V2reportsSequenceDataReportMatch.to_json())

# convert the object into a dict
v2reports_sequence_data_report_match_dict = v2reports_sequence_data_report_match_instance.to_dict()
# create an instance of V2reportsSequenceDataReportMatch from a dict
v2reports_sequence_data_report_match_from_dict = V2reportsSequenceDataReportMatch.from_dict(v2reports_sequence_data_report_match_dict)
```
[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


